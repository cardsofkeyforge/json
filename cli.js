...
async function processingDeck(deckId) {
    return Promise.resolve(await deckApi.retrieveDeck(deckId, configs.lang, false));
}

const importDeck = function(deckId, options) {
    console.info(chalk.blue(`Import deck: ${deckId} with ${options.cockatrice ? 'cocatrice' : 'standard'} output`));
    if (options.cockatrice) {
        processingDeck(deckId).then(deck => {
            const expansion = configs.expansion.find(e => e.code === deck.data.expansion);
            const jsons = './json/' + expansion.lang + '/' + expansion.name + '/';
            let n = 0;
            let c = '';
            const xmlCards = [];
            deck.data.cards.forEach(cardId => {
                const card = JSON.parse(fs.readFileSync(jsons + cardId + '.json').toString());
            ...
            ejs.renderFile('./decks/deck.ejs', {
                ...
                name: deck.data.name,
                cards: xmlCards
            }, {filename: deckId}, function(err, str) {
                if (!err) console.info(chalk.blue(str));
            });
        });
    } else {
        processingDeck(deckId).then(deck => console.info(chalk.blue(JSON.stringify(deck))));
    }
};
